        ...
        <div class="col-xs-12">
            <div class="box box-default">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans("posts::admin_lang.newpost") }}</h3>
                </div>

                <div class="box-body">
                    @if(Auth::user()->can("admin-posts-create"))
                        <a href="{{ url('admin/posts/create') }}" class="btn btn-success pull-right"><i class="fa fa-plus-circle" aria-hidden="true"></i> {{ trans('posts::admin_lang.nuevo_newpost') }}</a>
                    @endif
                </div>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            oTable = $('#table_posts').DataTable({
                ...
                columns: [
                    {
                        "title"         : "{!! trans('posts::admin_lang.visible') !!}",
                        orderable       : false,
                        searchable      : false,
                        data            : 'active',
                        sWidth          : '50px'
                    ...
                    {
                        "title"         : "{!! trans('posts::admin_lang.titulo') !!}",
                        orderable       : true,
                        searchable      : true,
                    ...
                    {
                        "title"         : "{!! trans('posts::admin_lang.url_seo') !!}",
                        orderable       : true,
                        searchable      : true,
                        data            : 'url_seo',name            : 'pt.url_seo',
                        sWidth          : ''
                    },
                    {
                        "title"         : "{!! trans('posts::admin_lang.fechaNoticia') !!}",
                        orderable       : true,
                        searchable      : true,
                        data            : 'date_post',name            : 'p.date_post',
                        sWidth          : '70px'
                    },
                    {
        ...
        function sendNotify(url) {
            ...
            $("#confirmModalLabel").html("{{ trans('posts::admin_lang.notify_warning_title') }}");
            $("#confirmModalBody").html("{{ trans('posts::admin_lang.notify_send_question') }}");
            strBtn+= '<button type="button" class="btn btn-default" data-dismiss="modal">{{ trans('general/admin_lang.close') }}</button>';
            strBtn+= '<button type="button" class="btn btn-primary" onclick="javascript:confirmSendNotify(this, \''+url+'\');">{{ trans('posts::admin_lang.notify_send') }}</button>';
            $("#confirmModalFooter").html(strBtn);
            $('#modal_confirm').modal('toggle');
        }

        function confirmSendNotify(btn, url) {

            btn.disabled=true;
            btn.innerHTML='<i class="fa fa-spinner fa-spin"></i> {{ trans('posts::admin_lang.notify_sending') }}';
            $.ajax({
                url     : url,
                type    : 'POST',
                "headers": {"X-CSRF-TOKEN": "{{ csrf_token() }}"},
                success : function(data) {
                    ...
                    if(data) {
                        ...
                        $("#alertModalHeader").html("{{ trans('posts::admin_lang.notify_send_posts') }}");
                        $("#alertModalBody").html("<i class='fa fa-check-circle' style='font-size: 64px; float: left; margin-right:15px;'></i> " + data.msg);
                        $("#modal_alert").modal('toggle');
                        oTable.ajax.reload(null, false);
                    } else {
                        $("#modal_alert").addClass('modal-danger');
                        $("#alertModalBody").html("<i class='fa fa-bug' style='font-size: 64px; float: left; margin-right:15px;'></i> {{ trans('posts::admin_lang.notify_send_error') }}");
                        $("#modal_alert").modal('toggle');
                    }
                    return false;
                }
