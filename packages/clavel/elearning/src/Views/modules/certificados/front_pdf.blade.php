                ...
                <div class="buttonsOfBlcok2" style="overflow: hidden; position: absolute; width:1123px; height:794px; @if(isset($certificado->paginasCertificado[$nY-1])) background-image: url({{ $certificado->paginasCertificado[$nY-1]->{'plantilla:'.$key} }}); @endif">
                    @if(isset($certificado->paginasCertificado[$nY-1]->translations->where("locale",$key)->first()->elementosPagina))
                        @foreach($certificado->paginasCertificado[$nY-1]->translations->where("locale",$key)->first()->elementosPagina as $keyEl=>$valEl)
                            <?php
                            $textCertificado = $valEl->name;

                            //Reemplazamos los textos
                            $textCertificado = str_replace("##alumno##", Auth::user()->userProfile->full_name, $textCertificado);
                            $textCertificado = str_replace("##curso##", $asignatura->titulo, $textCertificado);
                            $textCertificado = str_replace("##fechacertificado##", $tracking->fecha_fin_normal_formatted, $textCertificado);
                            $textCertificado = str_replace("##dni##", Auth::user()->userProfile->nif, $textCertificado);
                            ...
                            ?>
                            <div class="elementInfo" style="<?php echo $strStyle;?>">{!! $textCertificado !!}</div>
                        @endforeach
                    @endif
                </div>
                @endif
