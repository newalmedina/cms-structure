...
@section('breadcrumb')
    <li class="active">{{ trans('elearning::certificados/admin_lang.certificados') }}</li>
@stop
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <div class="box ">
                ...
                <div class="box-header"><h3 class="box-title">{{ trans("elearning::certificados/admin_lang.listado_cursos") }}</h3></div>

                <div class="box-body">
                    @if(Auth::user()->can("admin-certificados-create"))
                        <p>
                            <a href="{{ url('admin/certificados/create') }}" class="btn btn-primary">{{ trans('elearning::certificados/admin_lang.nuevo_pages') }}</a>
                        </p>
                    @endif

                    <table id="table_certificados" class="table table-bordered table-hover table-responsive"  style="width: 99.99%;" aria-hidden="true">
                        <thead>
                            <tr>
                                <th scope="col"></th>
                                <th scope="col"></th>
...
@section("foot_page")
    ...
    <script type="text/javascript">
        ...
        $(function () {
            oTable = $('#table_certificados').DataTable({
                "stateSave": true,
                "stateDuration": 60,
                "bProcessing": true,
                "bServerSide": true,
                ajax: {
                    ...
                    url         : "{{ url('admin/certificados/getData') }}",
                    type        : "POST"
                },
                order: [[ 1, "asc" ]],
                columns: [
                    {
                        "title"         : "{!! trans('elearning::certificados/admin_lang.activo') !!}",
                        orderable       : false,
                        searchable      : false,
                        data            : 'activo',
                        name            : 'activo',
                    ...
                    {
                        "title"         : "{!! trans('elearning::certificados/admin_lang.nombre') !!}",
                        orderable       : true,
                        searchable      : true,
                        data            : 'nombre',
                        name            : 'nombre',
                    ...
                    {
                        "title"         : "{!! trans('elearning::certificados/admin_lang.acciones') !!}",
                        orderable       : false,
                        searchable      : false,
                        sWidth          : '90px',
                        data            : 'actions'
        ...
        $('tfoot th',$('#table_certificados')).each( function (colIdx) {
            var title = $('tfoot th',$('#table_certificados')).eq( $(this).index() ).text();
        ...
        $('#table_certificados').on( 'keyup change','tfoot input', function (e) {
            oTable
