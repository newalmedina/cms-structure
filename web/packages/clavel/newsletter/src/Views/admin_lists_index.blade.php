        ...
        <div class="col-xs-12">
            <div class="box box-default">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans("Newsletter::admin_lang_lists.newsletter-lists") }}</h3>
                </div>

                <div class="box-body">
                    @if(Auth::user()->can("admin-newsletter-lists-create"))
                        <a href="{{ url('admin/newsletter-lists/create') }}" class="btn btn-success pull-right"><i class="fa fa-plus-circle" aria-hidden="true"></i> {{ trans('Newsletter::admin_lang_lists.nueva_list') }}</a>
                    @endif
                </div>

                <!-- /.box-header -->
                <div class="box-body">
                    <table id="table_lists" class="table table-bordered table-striped" aria-hidden="true">
                        <thead>
                        <tr>
                            <th scope="col"></th>
                            <th scope="col"></th>
        ...
        $(function () {
            oTable = $('#table_lists').DataTable({
                ...
                ajax: {
                    ...
                    url         : "{{ url('admin/newsletter-lists/list') }}",
                    type        : "POST"
                },
                order: [[ 1, "asc" ]],
                columns: [
                    {
                        "title"         : "{!! trans('Newsletter::admin_lang_lists.list') !!}",
                        orderable       : true,
                        searchable      : true,
                        data            : 'name', name            : 'name',
                        sWidth          : ''
                    ...
                    {
                        "title"         : "{!! trans('Newsletter::admin_lang_lists.subscriptions') !!}",
                        orderable       : true,
                        searchable      : true,
                    ...
                    {
                        "title"         : "{!! trans('Newsletter::admin_lang_lists.campaigns') !!}",
                        orderable       : true,
                        searchable      : true,
                    ...
                    {
                        "title"         : "{!! trans('Newsletter::admin_lang_lists.acciones') !!}",
                        orderable       : false,
                        searchable      : false,
                        sWidth          : '90px',
                        data            : 'actions'
            ...
            $('tfoot th',$('#table_lists')).each( function (colIdx) {
                var title = $('tfoot th',$('#table_lists')).eq( $(this).index() ).text();
            ...
            $('#table_lists').on( 'keyup change','tfoot input', function (e) {
                oTable
            ...
            $.ajax({
                ...
                success : function(data) {
                    ...
                    if(data) {
                        ...
                        $("#alertModalHeader").html("Borrado de list");
