...
@section('breadcrumb')
    <li class="active">{{ trans('timetracker::activities/admin_lang.list') }}</li>
@stop
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <div class="box ">
                ...
                <div class="box-header"><h3 class="box-title">{{ trans("timetracker::activities/admin_lang.list") }}</h3></div>

                <div class="box-body">
                    @if(Auth::user()->can("admin-activities-create"))
                        <p>
                            <a href="{{ url('admin/activities/create') }}" class="btn btn-primary">{{ trans('timetracker::activities/admin_lang.new') }}</a>
                        </p>
                    @endif

                    <table id="table_activities" class="table table-bordered table-hover" aria-hidden="true">
                        <thead>
                        <tr>
                            <th scope="col"></th>
                            <th scope="col"></th>
...
@section("foot_page")
    ...
    <script type="text/javascript">
        ...
        $(function () {
            oTable = $('#table_activities').DataTable({
                "stateSave": true,
                "stateDuration": 60,
                "bProcessing": true,
                "bServerSide": true,
                ajax: {
                    ...
                    url         : "{{ url('admin/activities/getData') }}",
                    type        : "POST"
                },
                order: [[ 1, "asc" ]],
                columns: [
                    {
                        "title"         : "{!! trans('timetracker::activities/admin_lang.active') !!}",
                        orderable       : false,
                        searchable      : false,
                        data            : 'active',
                        name            : 'active',
                    ...
                    {
                        "title"         : "{!! trans('timetracker::activities/admin_lang.name') !!}",
                        orderable       : true,
                        searchable      : true,
                        data            : 'name',
                    ...
                    {
                        "title"         : "{!! trans('timetracker::activities/admin_lang.actions') !!}",
                        orderable       : false,
                        searchable      : false,
                        sWidth          : '90px',
                        data            : 'actions'
            ...
            $('tfoot th',$('#table_activities')).each( function (colIdx) {
                var title = $('tfoot th',$('#table_activities')).eq( $(this).index() ).text();
            ...
            $('#table_activities').on( 'keyup change','tfoot input', function (e) {
                oTable
